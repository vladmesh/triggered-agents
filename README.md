...
--finalize`, обычным shell `;` (не `&&`) — значит trailer выполняется всегда, независимо от того,
успешно ли завершилась голова или с ошибкой. Как только сам процесс головы выходит, ЭТОТ ЖЕ
терминал немедленно останавливает себя (`dispatch.finalize` → `_stop_and_confirm_workspace_empty`
→ `_reap_ghosts`) — cleanup происходит в момент завершения, а не когда-то потом на очередном
опросе. `finalize` берёт тот же `state.lock()`, что и `run()`, поэтому не может состязаться с
параллельным тиком (если лок занят — просто откатывается, ничего не трогая; уборку доделает тот,
кто держит лок).
...
1. `state/curator/runs.jsonl` (или `TA_STATE/curator/runs.jsonl` на хосте) — последняя запись:
   ...
   `stray-sweep-failed`/`cleanup-stray-swept`/`cleanup-stray-sweep-failed`/`self-teardown`/
