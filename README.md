...
Так же строго с ghost-вкладками: `_reap_ghosts` возвращает `(closed, ok)`, где `ok = False`, если
`session.tabs.listAll` недоступен ИЛИ хотя бы один `session.tabs.close` упал — то есть
`pending-handle`-вкладка могла остаться в `session.tabs.listAll`. Ни один teardown-путь не пишет
здоровый `self-teardown`/`cleanup-teardown`/`ephemeral-restart`, пока `ok` не `True`: вместо этого
логируется соответствующий `*-tab-failed` action, а restart-пути (watchdog/ephemeral/stray-sweep
перед созданием) при `ok = False` вообще не поднимают свежий терминал этим тиком — чтобы не плодить
живой процесс рядом с незакрытой ghost-вкладкой. Это же касается и пути «нет живого терминала,
создаём свежий»: PTY завершённого прогона мог уже умереть (и `_agent_terminals`/`_raw_terminal_count`
читают пусто), но его `pending-handle`-вкладка ещё висит — если top-of-run реап вернул `ok = False`,
create-ветка тоже не поднимает новую сессию, а логирует `reap-tab-failed` (иначе новый куратор
запустился бы рядом с призраком завершённого). pty при этом уже остановлен, так что призрак
до-реапит top-of-run реап следующего тика. Единственное исключение — сам top-of-run реап
(оппортунистический prune в начале `run()`) НЕ обрывает тик при `ok = False`: он лишь фиксирует
`ok`, а уже конкретная ветка ниже (teardown, restart или create) решает, безопасно ли продолжать.
...
1. `state/curator/runs.jsonl` (или `TA_STATE/curator/runs.jsonl` на хосте) — последняя запись:
   ...
   `self-teardown-failed`/`*-tab-failed`/`*-stop-failed`/...), `ts` — когда. `self-teardown` — самый
   частый случай в здоровом воркспейсе: голова завершилась и её же терминал сам себя убрал через
   `--finalize`, без участия следующего тика. `self-teardown-superseded` — тоже здоровый:
   параллельный тик успел заменить терминал, и finalize корректно не тронул живую замену.
   `self-teardown-deferred` — finalize не смог взять лок за отведённое окно и отдал уборку тику,
   который его держит; редкий, но не ошибка. `*-check-failed`/`*-stop-failed` — `terminal
   list`/`stop` не отвечает или не убил pty, это аномалия уровня Orca/хоста, не куратора.
   `*-tab-failed` (`self-teardown-tab-failed`, `cleanup-teardown-tab-failed`,
   `ephemeral-restart-tab-failed`, `reap-tab-failed` и т.д.) — pty остановлен (или уже мёртв), но
   `session.tabs.close` не смог закрыть ghost-вкладку: тоже Orca-уровень; призрак до-реапит
   top-of-run реап следующего тика, а restart/create-путь в этот тик намеренно не поднимает свежую
   сессию рядом с призраком. `reap-tab-failed` конкретно — живого терминала уже нет, но старая
   `pending-handle`-вкладка не закрывается, поэтому новый прогон не стартует.
