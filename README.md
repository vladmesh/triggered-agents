...
- **board** (второй плагин) — сводит планы всех проектов в единый Kanboard-дашборд (swimlane на
  проект). PoC односторонний: проекты → доска, доска ридонли. Скилл — `.claude/skills/board`.
- **retro** (третий плагин) — ретро-пасс по свежим транскриптам голов (harvest переиспользует
