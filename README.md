...
живой процесс рядом с незакрытой ghost-вкладкой. pty при этом уже остановлен, так что призрак
до-реапит top-of-run реап следующего тика. Единственное исключение — этот самый top-of-run реап
(оппортунистический prune в начале `run()`): он игнорирует `ok`, потому что за ним всё равно идёт
решение тика, а реальный teardown-путь ниже сам перепроверит.
...
1. `state/curator/runs.jsonl` (или `TA_STATE/curator/runs.jsonl` на хосте) — последняя запись:
   ...
   `ephemeral-restart-tab-failed` и т.д.) — pty остановлен, но `session.tabs.close` не смог закрыть
   ghost-вкладку: тоже Orca-уровень; призрак до-реапит top-of-run реап следующего тика, а
   restart-путь в этот тик намеренно не поднимает замену.
