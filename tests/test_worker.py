...
class TerminalLiveTest(unittest.TestCase):
    ...
    def test_codex_tui_activity_uses_matching_session_mtime(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            match = sessions / "rollout-match.jsonl"
            other = sessions / "rollout-other.jsonl"
            match.write_text(json.dumps({
                "type": "session_meta",
                "payload": {"cwd": "/ws/fresh", "session_id": "match"},
            }) + "\n", encoding="utf-8")
            other.write_text(json.dumps({
                "type": "session_meta",
                "payload": {"cwd": "/ws/other", "session_id": "other"},
            }) + "\n", encoding="utf-8")
            os.utime(match, (1234.5, 1234.5))
    ...
    def test_codex_tui_session_mtime_does_not_revive_dead_handle(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            path.write_text(json.dumps({
                "type": "session_meta",
                "payload": {"cwd": "/ws/fresh", "session_id": "dead"},
            }) + "\n", encoding="utf-8")
            os.utime(path, (9999.0, 9999.0))

            with mock.patch.object(worker, "CODEX_SESSIONS", sessions), \
