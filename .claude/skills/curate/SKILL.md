...
фактов. `memory-write` может вернуть `op: "skip", reason: "already_exists"`: это успешный
исход, факт уже есть в каноне, можно продолжать. Если protocol вернул ошибку, не вызывай
`advance`: следующий прогон повторит commit того же proposal и не создаст дубль. Если
durable-фактов не нашлось, всё равно `advance`, чтобы не перечитывать те же ходы.
