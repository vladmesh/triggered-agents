...
Батч — из двух источников, по обеим головам (Claude и Hermes):
- **Транскрипты** — новые ходы сессий. У Claude это построчный JSONL за сессию, у Hermes —
  строки таблицы `messages` в его общей `~/.hermes/state.db` (не файлы `~/.hermes/sessions/
  session_*.json` — тот путь у текущей версии Hermes всегда пуст, сессии он хранит в SQLite).
- **Личная память голов** — новые/изменённые файлы `~/.claude/projects/*/memory/*.md`
  ...
  у таких записей `cwd` пустой, это ожидаемо, не баг.
...
`created` — дата хода из батча. `source` — голова (`claude`/`hermes`) и первые 8 символов
session id; для факта из личной памяти вместо session id — имя файла-источника
