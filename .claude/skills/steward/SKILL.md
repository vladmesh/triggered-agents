...
- `new_orphan_workspaces` — воркспейс на диске (`~/orca/workspaces/<project>/*`), которого нет в
  `state/pipeline/cards.json` — тик умер между созданием воркспейса и записью, либо teardown
  недоделался.
...
- **Эскалируешь.** Не можешь понять причину, или починка требует решения человека
  ...
  карточка «ушла другим путём», и сам подчищает свою запись в `cards.json` — досоздавать/чистить
